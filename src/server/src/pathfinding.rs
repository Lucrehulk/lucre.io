...
impl Pathfinder {
    ...
    pub fn find_path(&self, start_world: [f32; 2], end_world: [f32; 2]) -> Vec<[f32; 2]> {
        let sx = (start_world[0] / WALL_DIAMETER).floor() as i32; 
        let sy = (start_world[1] / WALL_DIAMETER).floor() as i32;
        let ex = (end_world[0] / WALL_DIAMETER).floor() as i32; 
        let ey = (end_world[1] / WALL_DIAMETER).floor() as i32;
        ...
        dist.insert(start, 0); 
        let h_start = (start.0 - goal.0).abs() + (start.1 - goal.1).abs(); heap.push(Node { cost: h_start, pos: start });
        while let Some(Node { cost: _, pos }) = heap.pop() {
            if pos == goal {
                let mut path = Vec::new(); let mut curr = goal;
                let end_center_x = (curr.0 as f32 * WALL_DIAMETER) + (WALL_DIAMETER / 2.0); 
                let end_center_y = (curr.1 as f32 * WALL_DIAMETER) + (WALL_DIAMETER / 2.0);
                path.push([end_center_x, end_center_y]);
                while let Some(prev) = came_from.get(&curr) { if *prev == start { break; }; 
                curr = *prev; 
                let wx = (curr.0 as f32 * WALL_DIAMETER) + (WALL_DIAMETER / 2.0); let wy = (curr.1 as f32 * WALL_DIAMETER) + (WALL_DIAMETER / 2.0); path.push([wx, wy]); }
                return path;
            }
